...
func CreateTask(db *sql.DB, task Task) error {
	...
	var err error
	query := "INSERT INTO tasks (name, completed) VALUES ($1, $2)"
...
func GetTaskCount(db *sql.DB) (int, error) {
	var result int
	if err := db.QueryRow("SELECT count(*) FROM tasks").Scan(&result); err != nil {
		return 0, err
	}
	return result, nil
}

func DeleteTasks(db *sql.DB) error {
...
func main() {
	...
	for _, task := range tasks {
		fmt.Printf("ID: %d. Name: %s. Completed: %s.", task.ID, task.Name, task.Completed)
	}

	if err := DeleteTasks(db); err != nil {
		fmt.Println(err)
		return
	}
